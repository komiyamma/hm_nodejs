            ...
            if (!initialized)
            {
                lock (syncRoot)
                {
                    if (!initialized)
                    {
                        ...
                        if (IntPtr.Size == 4)
                        {
                            LoadLibrary(AssemblyDirectory + @"\hmedgejs\x86\node.dll");
                            nodeStart = NodeStartx86;
                        }
                        else if (IntPtr.Size == 8)
                        {
                            LoadLibrary(AssemblyDirectory + @"\hmedgejs\x64\node.dll");
                        ...
                        Thread v8Thread = new Thread(() =>
                        {
                            ...
                            argv.Add(Path.Combine(AssemblyDirectory, "hmedgejs", "double_edge.js"));
                            argv.Add($"-EdgeJs:{Path.Combine(edgeDirectory, "hmEdgeJS.dll")}");
